...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!tokenResponse.ok) {
      ...
      return NextResponse.json({ error: 'GOOGLE REJECTED TOKEN EXCHANGE', details: errorText }, { status: 400 });
    }
    ...
    if(!existingUser?._id) {
        const newUser = await createNewAccount(userData?.email, userData?.picture, userData?.id);
