      ...
      <>
        {/* {userData?.profileUrl && ( */}
            <div className="aspect-w-1 aspect-h-1 w-8 rounded-full hover:cursor-pointer" onClick={() => setIsUserModal(true)}>
                <Image src={userData?.profileUrl ?? dummyProfile} alt={userData?.email || "profile"} className="w-full object-cover rounded-full" width={30} height={30}/>
            </div>
        {/* )} */}
        {isUserModal && (
            <>
                <div className="absolute z-50 top-10 right-0 bg-background border-2 border-secondary-foreground/30 flex flex-col rounded-lg rounded-tr-none">
                    ...
                    <Separator />
                    <button className="flex gap-4 p-4 items-center hover:cursor-pointer group" onClick={handleLogout} disabled={logoutLoading}>
                        <PiSignOut className="text-xl group-hover:text-chart-1" />
                        <p className="text-secondary-foreground group-hover:text-chart-1">Sign Out</p>
                    </button>
                </div>
                <div className="fixed z-40 w-screen h-screen top-0 right-0 p-8" onClick={() => setIsUserModal(false)}>
                </div>
            </>
        )}
      </>
